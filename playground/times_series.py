#https://github.com/oliverguhr/transformer-time-series-prediction?tab=readme-ov-file
...
def get_batch(input_data, i , batch_size):
    ...
    # torch.stack([item[0] for item in data]).shape = (batch_len, input_window)
...
def train(train_data):
    ...
    for batch, i in enumerate(range(0, len(train_data), batch_size)):  # Now len-1 is not necessary
        ...
        data, targets = get_batch(train_data, i , batch_size) # ( seq_len, batch, 1 ) , 1 is input feature size
        optimizer.zero_grad()
        output = model(data)
